...
jest.mock('ioredis', () => {
    const mRedis = {
        ...
        get: jest.fn(),
        ttl: jest.fn(),
        hset: jest.fn(), // Mocking the correct method name
        hGet: jest.fn(),
        hGetAll: jest.fn(),
...
describe('RedisClient', () => {
    ...
    test('✅ باید مقدار را از Redis دریافت کند', async () => {
        const key = 'test-key';
        (Redis.prototype.get as jest.Mock).mockResolvedValue('test-value');
    ...
    test('✅ باید مقدار `ttl` را دریافت کند', async () => {
        const key = 'test-key';
        (Redis.prototype.ttl as jest.Mock).mockResolvedValue(100);
    ...
    test('✅ باید مقدار `hget` را دریافت کند', async () => {
        const key = 'user:1';
        const field = 'name';
        (Redis.prototype.hget as jest.Mock).mockResolvedValue('John');
    ...
    test('✅ باید همه مقادیر را از `hgetall` دریافت کند', async () => {
        ...
        (Redis.prototype.hgetall as jest.Mock).mockResolvedValue(mockData);
