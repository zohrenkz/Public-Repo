...
describe('UserRepository', () => {
    ...
    it('should throw error if user is not found', async () => {
        ...
        await expect(userRepository.getById('invalid-id')).rejects.toThrowError(new CustomError('User not found.', 404, 'USER_NOT_FOUND'));
    });
    ...
    it('should throw an error if remove fails', async () => {
        ...
        await expect(userRepository.remove('mock-id')).rejects.toThrowError(new CustomError('Failed to remove user: Failed to remove', 500, 'USER_REMOVAL_FAILED'));
    });
});
